...
def create_data_row_styles(row, today_date):
    ...
    # Get date for this row with robust parsing
    date_str = row.get("Date", "")
    date_obj = None
    try:
        if date_str and str(date_str) != "":
            # Handle if already a Timestamp
            if isinstance(date_str, pd.Timestamp):
                date_obj = date_str
            else:
                # Try UK format explicitly first (DD/MM/YYYY)
                try:
                    date_obj = pd.to_datetime(date_str, format='%d/%m/%Y', errors='raise')
                except (ValueError, TypeError):
                    # Fallback: Let pandas infer with UK preference
                    date_obj = pd.to_datetime(date_str, dayfirst=True, errors='coerce')
                
                # If still failed, log it
                if pd.isna(date_obj):
                    from helpers import log_activity
                    log_activity(f"Date parsing failed for: {date_str}", level='warning')
    except Exception as e:
        # Silent fallback - styling will just skip this row
        pass
