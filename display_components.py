...
def show_legend(actual_visits_df):
    """Display legend for calendar interpretation - updated for VisitName"""
    legend_text = """
    ...
    **Actual Visits:**
    ...
    - ⚠ Screen Fail VisitName (Dark red background) = Screen failure (no future visits - only valid up to Day 1)
...
def display_calendar(calendar_df, site_column_mapping, unique_sites, excluded_visits=None):
    """Display the main visit calendar with styling - no changes needed for VisitName"""
...
def _generate_calendar_html_with_separators(styled_df):
    """Generate HTML calendar with month separators - no changes needed"""
...
def display_site_statistics(site_summary_df):
    """Display basic site summary statistics - no changes needed"""
    st.subheader("Site Summary")
    st.dataframe(site_summary_df, use_container_width=True)

def display_monthly_income_tables(visits_df):
    """Display monthly income analysis with tables only - NO CHARTS - no changes needed"""
...
def display_quarterly_profit_sharing_tables(financial_df, patients_df):
    """Display quarterly profit sharing analysis with tables and calculations - NO CHARTS - no changes needed"""
    ...
    # Get current weights
    weights = (
        st.session_state.list_weight / 100,
        st.session_state.work_weight / 100,
        st.session_state.recruitment_weight / 100
    )
    ...
    if quarterly_ratios:
        ...
        **Profit Sharing Formula:** 
        - List Sizes: {st.session_state.list_weight}% (Ashfields: {list_ratios['ashfields']:.1%}, Kiltearn: {list_ratios['kiltearn']:.1%})
...
def _display_weight_adjustment_interface():
    """Display the weight adjustment interface - no changes needed"""
...
def display_profit_sharing_ratio_breakdowns(financial_df, patients_df):
    """Display detailed ratio breakdowns for profit sharing calculations - no changes needed"""
...
def display_income_realization_analysis(visits_df, trials_df, patients_df):
    """Display income realization analysis section - no changes needed"""
...
def display_site_wise_statistics(visits_df, patients_df, unique_sites, screen_failures):
    """Display detailed statistics for each site with quarterly and financial year analysis - no changes needed"""
...
def _display_single_site_analysis(visits_df, patients_df, enhanced_visits_df, site, screen_failures):
    """Display comprehensive analysis for a single site - no changes needed"""
...
def _display_site_recruitment_analysis(site_patients, enhanced_visits_df, site):
    """Display patient recruitment analysis for a site - no changes needed"""
...
def _display_site_screen_failures(site_patients, screen_failures):
    """Display screen failures for a site - no changes needed"""
...
def display_download_buttons(calendar_df, site_column_mapping, unique_sites):
    """Display comprehensive download options with Excel formatting - no changes needed"""
