...
from formatters import (
    ...
    clean_numeric_for_display, apply_conditional_formatting
)
...
def display_profit_sharing_ratio_breakdowns(financial_df, patients_df):
    ...
    for period_key, period_config in time_periods.items():
        ...
        if ratio_data:
            st.write(f"**{period_config['title']}**")
...
def _display_single_site_analysis(visits_df, patients_df, enhanced_visits_df, site, screen_failures):
    ...
    with col1:
        st.metric("Total Patients", len(site_patients))
    with col2:
        st.metric("Total Visits", len(site_visits))
    with col3:
        st.metric("Completed Visits", len(site_visits[site_visits.get('IsActual', False)]))
    with col4:
        st.metric("Total Income", format_currency(site_visits['Payment'].sum()))
    ...
    # Time-based analysis
    _display_site_time_analysis(visits_df, patients_df, site, enhanced_visits_df)
...
def _display_site_time_analysis(visits_df, patients_df, site, enhanced_visits_df):
